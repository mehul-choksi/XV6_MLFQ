...
usertrap(void)
{
 ...
 if (r_scause() == 15){
    ...
    // Increment current running time for the process
    p->curr_run_time++;

    // Check if there are processes in lower priority queues and yield if necessary
    ...
    // Check if the process has exceeded its maximum allowable ticks in the current queue
    if (p->curr_run_time >= mlfq_queue.proc_queue_max_allowable_ticks[p->queue_num]) {
        // Promote the process to a higher priority queue if applicable
